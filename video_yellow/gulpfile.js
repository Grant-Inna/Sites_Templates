...
gulp.task('watch_JS', ['browser'], function() {
...
gulp.task('imageMINDir', function() {
    return gulp.src( 'src/img/content/*.{png,jpg,jpeg,svg}' )
        .pipe(imagemin())
        .pipe(gulp.dest( 'production/img/content/' ))
        .pipe(notify('Images Compress Success!'));
});
...
gulp.task('watch_imageMIN', function() {
    ...
    gulp.watch('src/img/content/*.{png,jpg,jpeg,svg}').on('change', browserSync.reload)
});
...
gulp.task('default', ['copyFonts', 'copyJS', 'watch_JS', 'CSS', 'jade', 'watch_JADE', 'watch_CSS', 'imageMIN', 'watch_imageMIN', 'imageMINDir']);
gulp.task('images', ['imageMIN', 'imageMINDir', 'watch_imageMIN']);
gulp.task('styles', ['CSS', 'watch_CSS']);